    ...
    public static void onLivingTick(LivingEntity entity) {
        ...
        AirProtectionSource protection = switch (o2Pair.getFirst()) {
            ...
            case YELLOW -> {
                ...
                } else if (prot == AirProtectionSource.RESPIRATOR && !entity.level.isClientSide && entity.level.getGameTime() % (20 * 15) == 0) {
                    CommonAbstractions.INSTANCE.findEquippedItem(entity, ModRegistry.RESPIRATOR_ITEM.get()).ifPresent(stack -> {
                        stack.hurt(1, entity.getRandom(), entity instanceof ServerPlayer player ? player : null);
                    });
                }
                yield prot;
            }
